...
$sql = "SELECT COUNT(*) AS notificationCount FROM report";

$result = $conn->query($sql);

if ($result && $result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $counterNot = $row['notificationCount']; // Assign the count value to $counterNot
} else {
...
<div class="notif-bell">
    ...
    <div class="main-container">
        <button class="DO-dashboard-button" onclick="redirectToDOviewReport()">
            <div class="title-container">
            <h3 style="text-align: center;">Messages
            </h3>
            <?php
                if($counterNot == 0){
                    echo '<h5>No Report to Review</h5>';
                } else  {
                    echo "<h5>You have a $counterNot to review</h5>";
                }
            ?>
              <!-- For Report  Dynamic-->  
            </h5>
        ...
        <button class="DO-dashboard-button" onclick="RedirectmeToSchedule()">
        <div class="title-container">
            <h3 style="text-align: center;">Schedule
            </h3>
            <?php
                echo '<h5>Set Schedule</h5>'
            ?> 
            <!-- For No Schedule  Dynamic-->
        </div>
        </button>
...
<script>
    ...
    function redirectToDOviewReport() {
        window.location.href = 'http://localhost:8080/DisciplinaryOffice/DOviewreports.php';
    }
